...
当头像需要展示动态信息或多个文字对象时，先拆分信息层级，再决定每个对象的区域、层级、字号和效果：

- 不同信息承担不同职责时，应分别渲染；主要信息可以更大、位置固定，辅助信息保持克制，避免把所有内容挤进一个字符串。
- 多行内容使用 JSON 字符串中的 `\\n`，并显式设置 `line_height`、`align` 和安全区域；不要依赖渲染器猜测换行。分行后检查总高度、水平居中和边缘留白。
- 文字区域应避开脸、手部和关键动作，并为描边、发光、阴影和旋转预留边界。最长内容和最宽字符必须作为压力测试样例。
- 如果文字位于具有材质的表面上，应匹配该表面的透视、曲率、光照、阴影和材质噪声，使文字看起来属于场景，而不是后贴的纯色标签。
- 对象较多时，先建立区域表，记录用途、层级、最大宽度、字号、行高、旋转和遮挡关系，避免对象相互覆盖或旧图层残留。

雪碧图适合固定数字、图标和需要逐位控制的视觉元素，但不适合替代任意文本。使用雪碧图时：

- 确认继承后的配置包含完整素材；不需要的旧 sprite 必须明确关闭，防止旧字符透出或与新文字叠加。
- “字体文件存在”不等于“字形可渲染”。对实际使用的字符集做 glyph 覆盖检查，并用目标渲染器预览，而不是只看字体名称。
- 需要统一视觉风格时可以复用同一字体，但不同信息仍应单独配置字号和位置；不要用改变画布比例或严重压缩字符来解决空间不足。

### 背景变体与情绪递进

需要普通背景和多个状态背景时，所有变体必须保持相同的画布、镜头、主体位置、主要留白和信息承载面。可以改变表情、眼神、动作、光线、色温或材质细节，但变化应连续、可解释且不破坏主体的自然感。

- 先确认普通背景，再以它作为构图锚点生成或编辑各状态；不要让生成模型重新设计镜头或移动留白区。
- 状态效果应单调递进：低级别只做轻微表情/色彩变化，高级别再叠加更明显的情绪、光线、材质或姿势变化。只在需求指定的级别加入特殊效果。
- 背景图本身不要绘制最终动态内容、测试文字或伪装字符；动态内容交给 renderer。生成模型经常会产生伪文字，必须放大检查所有可能承载信息的表面。
- “更靠近”应通过自然的身体前倾或轻微姿势变化实现，同时保持相同裁切和数字安全区，不能用改变镜头焦距或裁剪来破坏跨状态对齐。
- 身体结构和衣物调整应服务于自然性与角色气质；避免不必要的裸露、夸张肌肉、异常骨骼线条或将数字放在没有语义来源的衣服/皮肤上。

图像生成工具可能输出非目标尺寸。可以在交付前统一缩放到 1280×1280，但缩放后必须重新检查透明通道、PNG 尾部完整性、构图偏移和 renderer 读取结果。不要只依据工具返回的尺寸描述。
...
如果用户要求固定大小的测试图，使用 `--max-bytes` 生成实际 JPEG，并逐个检查文件大小。例如 20KB 测试应确认每张输出都不超过 20000 bytes，而不是只确认命令成功。预览文件建议放到用户指定的预览目录；源 PNG 和 manifest 不要混入临时渲染目录。

对每个状态至少抽查低、中、高负载样例；对逐值内容还应抽查映射边界、最长文本、含特殊符号的文本和最大位数显示。若 renderer 报 `unexpected end of file`，优先怀疑最近的图片转换/复制过程生成了截断 PNG：重新从原始生成文件写出目标 PNG，并再次用 `file`、图像库打开和 renderer 三重验证。
